# 转介绍数据来源 PRD

## 一、修订记录

| 版本号 | 修改日期 | 修改原因 | 修订人 |
|--------|----------|----------|--------|
| v1.0 | 2026-03-24 | 初稿 | AI PRD Writer |

## 二、需求概述

### 背景/现状

当前 CRM 系统「客户管理 > 转介绍线索」（`/WH_CRM_v2/customer/referralClue/index`）页面主要用于管理转介绍产生的线索信息。但运营和管理人员缺少一个专门查看转介绍数据来源明细的入口，无法快速了解每一条转介绍记录的来源渠道（海报或小程序）、绑定关系以及归属情况。这导致在分析转介绍活动效果、追踪来源渠道贡献时，需要通过多处数据拼接，效率低下。

### 需求目标

1. 在「客户管理 > 转介绍线索」同层级新增一个页面「转介绍数据来源」，路径为 `/WH_CRM_v2/customer/referralSource/index`。
2. 提供完整的转介绍数据来源明细列表，包含业务线、客户手机号、新客户手机号、绑定时间、归属坐席、介绍人、来源渠道等关键字段。
3. 支持多维度筛选（手机号精准搜索、时间范围、业务线、归属坐席模糊搜索、来源渠道），方便运营人员快速定位目标数据。

### 核心逻辑简述

新增「转介绍数据来源」页面作为「转介绍线索」的同层级菜单项。页面提供筛选区和数据列表两个核心区域。筛选区支持客户手机号/新客户手机号精准搜索、时间范围选择（精确到秒）、业务线下拉枚举（电销/体验营）、归属坐席远程模糊搜索下拉选择、来源渠道下拉枚举（海报/小程序）。数据列表展示转介绍绑定关系明细，支持分页查看。后端根据筛选条件查询转介绍绑定记录并返回列表数据。

## 三、术语表

| 术语 | 定义 |
|------|------|
| 转介绍 | 老客户（介绍人）将新客户推荐给平台的行为，建立老客户与新客户之间的绑定关系 |
| 介绍人 | 发起转介绍行为的老客户，通过分享海报或小程序将新客户引入平台 |
| 客户手机号 | 介绍人（老客户）在平台注册时使用的手机号码 |
| 新客户手机号 | 被介绍的新用户在平台注册时使用的手机号码 |
| 绑定时间 | 新客户通过转介绍链路完成与介绍人绑定关系建立的时间点 |
| 归属坐席 | 该条转介绍记录所归属的电销/体验营坐席人员 |
| 来源 | 转介绍的渠道来源，区分为海报（用户扫描分享海报上的二维码）和小程序（用户通过小程序内分享入口） |
| 业务线 | 该条转介绍记录所属的业务线，分为电销和体验营 |

## 四、调整范围

| 序号 | 功能模块 | 调整说明 |
|------|----------|----------|
| 1 | 左侧导航菜单 | 新增：在「客户管理 > 转介绍线索」同层级新增「转介绍数据来源」菜单项 |
| 2 | 转介绍数据来源页面 — 筛选区 | 新增：包含客户手机号/新客户手机号输入、时间范围选择、业务线下拉、归属坐席模糊搜索下拉、来源下拉 |
| 3 | 转介绍数据来源页面 — 数据列表 | 新增：展示业务线、客户手机号、新客户手机号、绑定时间、归属坐席、介绍人、来源 7 个字段 |
| 4 | 转介绍数据来源页面 — 分页 | 新增：列表底部分页器，默认每页 20 条 |

## 五、业务流程图

```mermaid
flowchart TD
    A[用户点击左侧导航「转介绍数据来源」] --> B[页面加载，展示筛选区 + 默认数据列表]
    B --> C{用户是否设置筛选条件？}
    C -->|否| D[展示全量数据，按绑定时间倒序排列]
    C -->|是| E[用户填写筛选条件]
    E --> F{用户选择了哪些筛选项？}
    F -->|客户手机号/新客户手机号| G[精准匹配：完整手机号全匹配查询]
    F -->|时间范围| H[范围匹配：绑定时间在起止时间范围内]
    F -->|业务线| I[枚举匹配：筛选指定业务线记录]
    F -->|归属坐席| J[远程模糊搜索：输入关键字查询坐席列表 → 选中后精确匹配]
    F -->|来源| K[枚举匹配：筛选指定来源渠道]
    G & H & I & J & K --> L[用户点击「查询」按钮]
    L --> M[后端接口根据多条件组合查询]
    M --> N{是否有匹配数据？}
    N -->|是| O[展示筛选结果列表，支持分页]
    N -->|否| P[展示空状态：暂无数据]
    O --> Q{用户是否翻页？}
    Q -->|是| R[请求对应页码数据，刷新列表]
    Q -->|否| S[用户点击「重置」按钮]
    S --> B
    D --> Q
```

## 六、可交互原型

在线预览：[点击查看可交互原型](placeholder)

> 原型为可交互 HTML 页面，点击链接即可在浏览器中体验完整交互流程。

## 七、功能需求详细描述

### 7.1 页面整体布局

页面结构从左到右、从上到下依次为：

1. **左侧导航栏**：CRM 系统标准侧边栏，「转介绍数据来源」作为「客户管理」下的菜单项，与「转介绍线索」同层级
2. **顶部面包屑**：客户管理 > 转介绍数据来源
3. **筛选区**：包含多个筛选组件，横向排列自动换行
4. **数据列表区**：展示转介绍数据明细表格
5. **分页器**：列表底部分页控件

### 7.2 左侧导航菜单调整

| 功能按钮 | 功能说明 | 是否必填 | 功能类型 | 交互说明 | 备注 |
|----------|----------|----------|----------|----------|------|
| 转介绍数据来源 | 进入转介绍数据来源列表页 | — | 菜单项 | 点击后右侧内容区切换为转介绍数据来源页面；菜单项高亮 | 位于「客户管理」分组下，与「转介绍线索」同层级 |

### 7.3 筛选区

| 功能按钮 | 功能说明 | 是否必填 | 功能类型 | 交互说明 | 备注 |
|----------|----------|----------|----------|----------|------|
| 客户手机号 | 按介绍人手机号精准搜索 | 否 | 文本输入框 | 输入完整手机号后点击查询，后端进行精准匹配 | placeholder: "请输入客户手机号"；仅支持 11 位数字输入 |
| 新客户手机号 | 按被介绍新客户手机号精准搜索 | 否 | 文本输入框 | 输入完整手机号后点击查询，后端进行精准匹配 | placeholder: "请输入新客户手机号"；仅支持 11 位数字输入 |
| 时间范围 | 按绑定时间范围筛选 | 否 | 日期时间范围选择器 | 选择开始时间和结束时间，精确到秒（YYYY-MM-DD HH:mm:ss）；结束时间不得早于开始时间 | 默认不选择，即查询全部时间段；选择后自动校验时间先后顺序 |
| 业务线 | 按业务线筛选 | 否 | 下拉选择器 | 单选；枚举值：全部 / 电销 / 体验营 | 默认"全部" |
| 归属坐席 | 按归属坐席筛选 | 否 | 远程搜索下拉选择器 | 用户输入关键字（≥2 个字符）触发远程模糊查询，返回匹配的坐席列表供选择；选中后作为精确条件查询 | placeholder: "请输入坐席姓名搜索"；API: `/api/agent/search?keyword=xxx` |
| 来源 | 按来源渠道筛选 | 否 | 下拉选择器 | 单选；枚举值：全部 / 海报 / 小程序 | 默认"全部" |
| 查询 | 触发筛选查询 | — | 按钮（主按钮） | 点击后根据当前所有筛选条件组合查询，刷新数据列表，页码重置为第 1 页 | — |
| 重置 | 清空所有筛选条件 | — | 按钮（默认按钮） | 点击后清空所有筛选输入/选择项，恢复默认值，刷新数据列表 | — |

**筛选交互规则：**

- 所有筛选条件为 AND 关系，多条件组合查询
- 点击「查询」按钮才会触发请求，输入过程中不自动搜索（归属坐席的下拉候选列表除外）
- 点击「重置」按钮后，所有筛选项恢复到默认状态（输入框清空，下拉选择器回到"全部"），并自动触发一次查询刷新列表
- 时间范围选择器选择后：
  - 如果只选了开始时间未选结束时间，查询开始时间之后的所有记录
  - 如果只选了结束时间未选开始时间，查询结束时间之前的所有记录
  - 如果结束时间早于开始时间，前端 toast 提示"结束时间不能早于开始时间"，不发起请求

### 7.4 数据列表

| 字段名称 | 字段说明 | 字段来源 | 备注 |
|----------|----------|----------|------|
| 业务线 | 该条转介绍记录所属业务线 | 后端返回 | 枚举值：电销 / 体验营 |
| 客户手机号 | 介绍人（老客户）的手机号 | 后端返回 | 11 位手机号，中间 4 位脱敏显示为 `***`，如 138****5678；鼠标 hover 时 tooltip 展示完整号码 |
| 新客户手机号 | 被介绍的新用户手机号 | 后端返回 | 11 位手机号，中间 4 位脱敏显示为 `***`，如 139****1234；鼠标 hover 时 tooltip 展示完整号码 |
| 绑定时间 | 新客户与介绍人完成绑定的时间 | 后端返回 | 格式：YYYY-MM-DD HH:mm:ss |
| 归属坐席 | 该记录归属的坐席人员姓名 | 后端返回 | 展示坐席姓名 |
| 介绍人 | 老客户（介绍人）的姓名或昵称 | 后端返回 | 如无姓名则展示"—" |
| 来源 | 转介绍来源渠道 | 后端返回 | 枚举值：海报 / 小程序；分别使用蓝色/绿色 tag 标签展示 |

**列表交互规则：**

- 默认按绑定时间倒序排列（最新的在最前面）
- 绑定时间列支持点击表头排序（升序/降序切换）
- 表格无数据时展示空状态占位图，文案"暂无数据"
- 表头固定，列表区域支持纵向滚动

### 7.5 分页器

| 功能 | 说明 |
|------|------|
| 位置 | 列表底部右侧 |
| 默认每页条数 | 20 条 |
| 每页条数可选 | 10 / 20 / 50 / 100 |
| 展示内容 | 共 X 条数据，当前页/总页数，页码按钮 |
| 翻页 | 点击页码或上下页按钮，请求对应页码数据刷新列表 |
| 筛选后分页 | 筛选条件变化时，页码自动重置为第 1 页 |

### 7.6 接口设计要求

**列表查询接口：**

- 请求方式：POST
- 路径：`/api/referralSource/list`
- 请求参数：

| 参数名 | 类型 | 必填 | 说明 |
|--------|------|------|------|
| customerPhone | String | 否 | 客户手机号，精准匹配 |
| newCustomerPhone | String | 否 | 新客户手机号，精准匹配 |
| startTime | String | 否 | 开始时间，格式 YYYY-MM-DD HH:mm:ss |
| endTime | String | 否 | 结束时间，格式 YYYY-MM-DD HH:mm:ss |
| businessLine | String | 否 | 业务线，枚举：TELEMARKETING / EXPERIENCE_CAMP |
| agentId | Long | 否 | 归属坐席 ID |
| source | String | 否 | 来源，枚举：POSTER / MINI_PROGRAM |
| pageNum | Integer | 是 | 当前页码，从 1 开始 |
| pageSize | Integer | 是 | 每页条数 |

- 返回参数：

| 参数名 | 类型 | 说明 |
|--------|------|------|
| total | Long | 总条数 |
| list | Array | 数据列表 |
| list[].businessLine | String | 业务线 |
| list[].customerPhone | String | 客户手机号（完整） |
| list[].newCustomerPhone | String | 新客户手机号（完整） |
| list[].bindTime | String | 绑定时间 |
| list[].agentName | String | 归属坐席姓名 |
| list[].referrerName | String | 介绍人姓名/昵称 |
| list[].source | String | 来源渠道 |

**坐席模糊搜索接口：**

- 请求方式：GET
- 路径：`/api/agent/search`
- 请求参数：

| 参数名 | 类型 | 必填 | 说明 |
|--------|------|------|------|
| keyword | String | 是 | 搜索关键字，≥2 个字符 |

- 返回参数：

| 参数名 | 类型 | 说明 |
|--------|------|------|
| list | Array | 坐席列表 |
| list[].agentId | Long | 坐席 ID |
| list[].agentName | String | 坐席姓名 |

## 八、埋点与数据需求

| 事件名称 | 触发时机 | 关键参数 | 用途 |
|----------|----------|----------|------|
| referral_source_page_view | 用户进入转介绍数据来源页面 | userId | 统计页面访问量和活跃用户 |
| referral_source_search | 用户点击「查询」按钮 | filterParams（包含所有筛选条件）、resultCount（查询结果条数） | 分析筛选使用习惯和数据匹配率 |
| referral_source_reset | 用户点击「重置」按钮 | userId | 分析重置频率，判断筛选项是否合理 |
| referral_source_page_change | 用户切换分页 | pageNum、pageSize | 分析用户浏览深度 |

## 九、权限说明

| 角色 | 权限范围 |
|------|----------|
| 运营管理员 | 可查看全部业务线的转介绍数据来源；可使用全部筛选条件 |
| 电销主管 | 可查看电销业务线的转介绍数据来源；归属坐席范围限定为其下属坐席 |
| 体验营主管 | 可查看体验营业务线的转介绍数据来源；归属坐席范围限定为其下属坐席 |
| 普通坐席 | 仅可查看自己归属的转介绍数据来源；归属坐席筛选项不展示 |
| 数据分析 | 可查看全部数据；可使用全部筛选条件 |

> **备注**：权限控制在接口层面实现，前端根据当前登录用户角色动态显示/隐藏筛选项并在请求中自动附加权限参数。普通坐席角色下，归属坐席筛选项隐藏，后端接口自动过滤为当前用户的数据。
